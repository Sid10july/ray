...
from ray.rllib.core.rl_module import RLModuleSpec, MultiRLModuleSpec
from ray.rllib.env.multi_agent_env_runner import MultiAgentEnvRunner
from footsies.fixed_rlmodules import (
    NoopFixedRLModule,
    BackFixedRLModule,
)
from footsies.footsies_env import (
    env_creator,
)
from footsies.utils import (
    Matchup,
    Matchmaker,
    MetricsLoggerCallback,
    MixManagerCallback,
...
# --- Ensure workers use this conda env and not user-site packages ---
import os, sys, numpy as _np
os.environ["PYTHONNOUSERSITE"] = "1"             # block ~/.local leaks
os.environ["RAY_PYTHON_EXECUTABLE"] = sys.executable  # force worker python
print("Driver Python:", sys.executable)
print("Driver NumPy:", _np.__version__)
...
if __name__ == "__main__":
    algo = config.build()
    try:
        # simple loop that matches your stop dict
        while True:
            result = algo.train()
            # optional: print the simple progress line
            tr = result.get("episode_reward_mean")
            er = result.get("evaluation", {}).get("episode_reward_mean")
            it = result.get("training_iteration")
            if tr is not None and er is not None:
                print(f"iter={it} R={tr:.1f} R(eval)={er:.1f}")
            # honor the same stop conditions
            if (
                result.get("mix_size", 0) >= args.target_mix_size
                or result.get("training_iteration", 0) >= args.stop_iters
                or result.get("num_env_steps_sampled_lifetime", 0) >= args.stop_timesteps
            ):
                break
    finally:
        # This is the important part: stop trainer/workers first.
        try:
            algo.stop()
        except Exception:
            pass
